...
export const ProductOptionsUI = (props) => {
  ...
  return (
    <ScrollView style={styles.mainContainer}>
      ...
      <WrapContent>
        <ProductTitle>
          <OText size={20} style={{ flex: 1 }}>{product.name}</OText>
          <OText size={20} color={colors.primary}>{parsePrice(productCart.price)}</OText>
        </ProductTitle>
        <ProductDescription>
          <OText>{product.description}</OText>
          <OText size={20}>{t('SKU', 'Sku')}</OText>
          <OText>{product?.sku}</OText>
        </ProductDescription>
        <ProductEditions>
