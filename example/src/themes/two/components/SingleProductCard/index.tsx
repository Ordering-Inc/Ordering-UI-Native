...
export const SingleProductCard = (props: SingleProductCardParams) => {
	...
	return (
		...
		>
			<CardInfo>
				<OText numberOfLines={1} ellipsizeMode='tail' style={{...styles.textStyle, ...labels.middle} as TextStyle}>{product?.name}</OText>
				<OText color={colors.textSecondary} numberOfLines={2} ellipsizeMode='tail' style={{...styles.textStyle, ...labels.normal} as TextStyle}>{product?.description}</OText>
				<OText color={colors.textThird} style={labels.normal as TextStyle}>{parsePrice(product?.price)}</OText>
			</CardInfo>
			...
			<View style={styles.soldOutBackgroundStyle}>
				<SoldOut>
					<OText weight={'bold'} color={colors.white} style={styles.soldOutTextStyle}>{t('SOLD_OUT', 'SOLD OUT')}</OText>
				</SoldOut>
			</View>
			)}
		</CardContainer>
