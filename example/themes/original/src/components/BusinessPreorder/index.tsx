...
const BusinessPreorderUI = (props: any) => {
  ...
  }, [selectDate, hoursList])

  useEffect(() => {
    if (selectedPreorderType.key === 'business_menu' && Object.keys(menu).length > 0) setMenu({})
  ...
  return (
    <>
      <PreOrderContainer>
        <TouchableOpacity onPress={() => onClose && onClose()} style={{ marginBottom: 12 }}>
          <IconAntDesign
            name='close'
