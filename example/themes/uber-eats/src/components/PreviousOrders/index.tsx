          ...
          <Information>
            ...
            <MyOrderOptions>
              <TouchableOpacity onPress={() => handleClickViewOrder(order?.uuid)}>
                <OText size={16} color={theme.colors.green} mRight={5} numberOfLines={1}>{t('MOBILE_FRONT_BUTTON_VIEW_ORDER', 'View order')}</OText>
              </TouchableOpacity>
              {
                allowedOrderStatus.includes(parseInt(order?.status)) && !order.review && (
                  <TouchableOpacity onPress={() => handleClickOrderReview(order)}>
                    <OText size={16} color={theme.colors.green} numberOfLines={1} style={{ textAlign: 'left' }}>{t('REVIEW_ORDER', 'Review Order')}</OText>
                  </TouchableOpacity>
                )}
            </MyOrderOptions>
          </Information>
